...
// SaveAsSVG saves the histogram as SVG using ECharts
func (hr *HistogramResult) SaveAsSVG(filename string) error {
	if !filepath.IsAbs(filename) {
		filename = filepath.Join(hr.Config.OutputDir, filename)
	}
...
// SaveAsPNG saves the histogram as PNG using ECharts (via HTML to PNG conversion)
func (hr *HistogramResult) SaveAsPNG(filename string) error {
	// For PNG output, we'll create an HTML file first and then use external tools
	// This is a common approach since ECharts requires a browser for PNG rendering
	htmlFile := strings.TrimSuffix(filename, ".png") + ".html"
	if err := hr.SaveAsHTML(htmlFile); err != nil {
		return err
	}

	// Note: In production, you would use a headless browser like Chrome Headless
	// to convert HTML to PNG. This is left as an exercise for the user.
	fmt.Printf("HTML file created: %s. Use external tools to convert to PNG.\n", htmlFile)
	return nil
}
